...
angular.module('didiApp')
  .controller('MainCtrl', function ($scope, $interval, localStorageService) {
    ...
    var forceOpenNewTaskForm = function() {
      if ($scope.tasks.length === 0 && $scope.showNewTaskForm === false) {
    ...
    var clearOnLoad = function() {
      ...
      for (var i = 0; i < $scope.tasks.length; i++) {
        completedAt = $scope.tasks[i].completedAt.getDate() + '-' + $scope.tasks[i].completedAt.getMonth() + '-' + $scope.tasks[i].completedAt.getFullYear();
